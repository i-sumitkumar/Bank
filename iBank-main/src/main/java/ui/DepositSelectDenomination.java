/*
 * Created by JFormDesigner on Thu Jul 04 01:44:19 EDT 2024
 */
 ...
 * @author jdiaz
 */
public class DepositSelectDenomination extends AbstractMenuPanel {
    ...
    public DepositSelectDenomination(MenuPanel menuPanel, Account account) {
        super(menuPanel);
    ...
    private void deposit(ActionEvent e) {
        try {
            ...
            Customer customer = this.getMenuPanel().getUserService().deposit(account.getAccountNumber(),
                    Double.parseDouble(this.totalAmount.getText()));
            atm.depositCash(denominationMap);
            getMenuPanel().setCustomer(customer);
            getMenuPanel().getAtmService().logATMDeposit(atm.getId(), customer.getId(), denominationMap);

            DepositResult result = new DepositResult(getMenuPanel(), account.getAccountType());
            refreshPanel(result);
        } catch (UserNotFoundException | ATMCashFullException ex) {
            showMessageDialog(this, ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            System.out.println("Error in deposit " + ex.getMessage());
        }
    }
    ...
    private void initComponents() {
        ...
        //---- button1 ----
        button1.setText("OK");
        button1.addActionListener(e -> deposit(e));
